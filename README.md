...
## Important limitations — please read before any clinical use

- **This is monocular RGB pose estimation, not multi-camera motion capture.**
  Your thesis title says "three-dimensional motion analysis," which in the
  literature usually implies a marker-based or multi-camera system (e.g.
  Vicon, OptiTrack). A single phone/webcam camera, even with MediaPipe's
  depth-ish `z` coordinate, is estimating 3D pose from one 2D view. Treat every
  angle here as a **candidate measurement to validate**, not a substitute for
  your reference standard, before drawing any clinical conclusion in the
  thesis.
- **Scapular parameters are the weakest link.** True scapular kinematics
  (winging, tilt, upward rotation) are usually assessed with skin markers,
  inertial sensors, or direct palpation — the scapula itself is not a pose
  landmark MediaPipe (or any off-the-shelf RGB pose model) tracks. This
  prototype is deliberately honest about that: it reports proxies and flags,
  not measurements, and routes them to clinician confirmation on the photo.
- **This prototype stores PHI (photos of children) in the browser's local
  IndexedDB only.** That is fine for a development demo but is *not* a
  finished, regulator-ready storage solution. A real deployment needs: an
  encrypted backend, an audit log, consent/assent workflows appropriate for
  pediatric research, and likely IRB/ethics-committee sign-off plus applicable
  medical-device regulatory review (e.g. FDA/CE) depending on how the tool is
  used and marketed.
- **On patenting:** I'm not a patent attorney and this isn't legal advice. The
  four theta-architecture properties above are implemented as genuine,
  specific software mechanisms (not just described in prose), which is the
  kind of concrete detail patent claims usually need — but novelty and
  non-obviousness against the existing patent landscape can only be confirmed
  by a professional prior-art search. I'd recommend having a patent attorney
  (or a patent agent who handles software/medtech) review this before filing,
  particularly to search for prior art around adaptive/versioned clinical
  scoring systems and monocular pose-based ROM measurement tools.

## Suggested next steps for the thesis

1. Validate each angle against your gold-standard measurement (goniometer,
   or whatever 3D system your methods section specifies) on a pilot sample,
   and report agreement (e.g. ICC, Bland-Altman) rather than assuming
   equivalence.
2. Replace the placeholder `NORMS` targets in `capture/app.js` and the
   placeholder weights in `config/theta-config.v1.json` with values fitted on
   your own cohort.
3. Add a real backend once you move past prototyping, and have your
   institution's ethics board review the consent and data-handling workflow
   given the participants are children.
